"""Local dense retrieval over the HotpotQA candidate contexts.

The retriever is deliberately small and local:
...
* embeddings = ``BAAI/bge-small-en-v1.5`` (sentence-transformers, CPU), L2
  normalised so that inner-product search equals cosine similarity;
* index = FAISS ``IndexFlatIP``, one index per question and granularity.
